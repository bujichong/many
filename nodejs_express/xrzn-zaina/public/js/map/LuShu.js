...
(function() {
    ...
    LuShu.prototype.setIndex = function (newI) {
        this.i = newI;
    };
     LuShu.prototype.updateSpeeds = function (speedArr) {
         this._opts.speeds = speedArr;
     }
    /**
     * @description 开始运动
     * @param none
    ...
    LuShu.prototype.start = function() {
        var me = this,
            len = me._path.length;
            //window.console && console.log(me._marker);
        //不是第一次点击开始,并且小车还没到达终点
        //window.console && console.log(this.i);
        if (me.i && me.i < len - 1) {
            //没按pause再按start不做处理
            if (!me._fromPause) {
                return;
            ...
            if (me._opts.marker) {//如果有自设定maker点
                me._marker = me._opts.marker;
                me.initRotation();
            }else{
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        initRotation : function () {
            if (this._path.length==2) {
                var path = this._path;
                //window.console && console.log(this._path);
                this.setRotation(null,path[0],path[1]);
            };
        },
        /**
         * 添加上方overlay
         ...
         * @return 无返回值.
         */
        _move: function(index,effect) {
            var me = this,
                //当前的帧数
                currentCount = 0,
                //步长，米/秒
                timer = 10,
                step = this._opts.speeds[index] / (1000 / timer),
                //初始坐标
                initPos = me._path[index],
                targetPos = me._path[index + 1],
                init_pos = this._projection.lngLatToPoint(initPos),
                //获取结束点的(x,y)坐标
                target_pos = this._projection.lngLatToPoint(targetPos),
                //总的步长
                count = Math.round(me._getDistance(init_pos, target_pos) / step);
                me._opts.movingBack&&me._opts.movingBack(index);
            //如果小于1直接移动到下一点
            if (count < 1) {
                me._moveNext(++me.i);
                return;
        ...
        */
        ...
        _moveNext: function(index) {
            ...
            if (index < this._path.length - 1) {
                me._move(index, me._tween.linear);
            }else{
                this._opts.endBack(this._marker);
            }
        },
        /**
