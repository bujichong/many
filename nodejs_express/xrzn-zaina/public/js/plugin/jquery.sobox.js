...
//			obj.css({'width':0,'height':0,'marginLeft':0,'marginTop':0}).show().animate({'width':w,'height':h,'marginLeft':-w/2,'marginTop':-h/2},'fast');
		...
		setPos:function(o) {//位置函数
			...
			switch (o.mode) {
				...
				break;
				case 'obj' ://x轴居中，定位在window bottom，不兼容ie6随屏幕滚动
					var ol = $(o.posObj).offset().left,ot = $(o.posObj).offset().top;
					$o.css({'position':'absolute','left':ol+offX,'top':ot+offY});
				break;
				default ://center
				...
				if (o.btn.length>0) {//添加按钮
					...
					$.each(o.btn,function () {
						...
						thisBtn.bind('click',function () {
							if (param.callback) {
								param.callback($wrap,thisBtn)&&removePop();
							}
		...
		alert : function (title,content,callback) {//提示框：快捷3个参数：标题、正文、点击确定返回事件
			...
			that.pop({
				cls:'so-popAlert',title:title,width:320,content:content,
				showTitle : showTitle,
				btn:[{text:'确定'}],
				closePop : callback
			});
		...
		confirm : function (title,content,success,cancel) {//确认框：快捷4个参数：标题、正文、点击确定返回事件、点击取消返回事件
			...
			that.pop({
				...
				showTitle :false,
				maskClick:false,
				title :title,width:288,content :content,
		...
		tip : function (o) {
			...
			var o = $.extend({
				...
				showMask :false,
				width:250,
				stayTime : 3000,//停留显示时间，不自动关闭设置为小于0
				offset:[0,5],
				closePop : function () {
		...
		*/
		...
		'clickPop' : function (o) { // 点击pop，sobox参数extend 从 optAll[aim] 中
			...
			_self.click(function () {// handler设置事件
				...
				//window.console && console.log(typeof(data));
				...
				if (aim) {
					...
					optT.beforePop = function($wrap,$title,$close,$cont){
						var $form = $wrap.find('form');
						$form.length&&$form.attr('rel',aim);
						opt.beforePop&&opt.beforePop.call(that,data,$wrap,$title,$close,$cont);
					};
